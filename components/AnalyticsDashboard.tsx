...
const FilterBadge: React.FC<{ label: string; active: boolean; onClick: () => void }> = ({ label, active, onClick }) => (
  <button 
    onClick={onClick}
    className={`px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider transition-all border ${active ? 'bg-indigo-600 text-white border-indigo-600 shadow-md' : 'bg-white dark:bg-slate-700 text-slate-500 dark:text-slate-300 border-slate-200 dark:border-slate-600 hover:border-indigo-300'}`}
  >
    {label}
  </button>
);
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
      <header className="mb-8">
        ...
        <p className="text-slate-500 dark:text-slate-300 font-medium">Tracking error patterns and quality trends across all verified sessions.</p>
      </header>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700">
          <h3 className="text-xs font-black text-slate-400 dark:text-slate-300 uppercase mb-8 tracking-widest">Error Types Distribution</h3>
          <div className="h-[280px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={barData}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" className="dark:stroke-slate-700" />
                <XAxis dataKey="name" fontSize={9} axisLine={false} tickLine={false} tick={{ fontWeight: 700, fill: 'var(--tw-text-slate-500)' }} />
                <YAxis fontSize={9} axisLine={false} tickLine={false} tick={{ fontWeight: 700, fill: 'var(--tw-text-slate-500)' }} />
                <Tooltip 
        ...
        <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700">
          <h3 className="text-xs font-black text-slate-400 dark:text-slate-300 uppercase mb-8 tracking-widest">Severity Overview</h3>
          <div className="h-[280px]">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
          ...
          <div className="flex justify-center flex-wrap gap-4 mt-4">
            {pieData.map((d, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black text-slate-500 dark:text-slate-300 uppercase tracking-tighter">{d.name}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="relative z-10">
            ...
            <div className="mt-10 space-y-3">
               <div className="bg-slate-800/80 backdrop-blur px-5 py-4 rounded-2xl flex justify-between items-center border border-slate-700/50 dark:border-slate-600/50">
                  <span className="text-[11px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-wider">Critical Failures</span>
                  <span className="text-base font-black text-red-400">{severityCount['HIGH'] || 0}</span>
               </div>
               <div className="bg-slate-800/80 backdrop-blur px-5 py-4 rounded-2xl flex justify-between items-center border border-slate-700/50 dark:border-slate-600/50">
                  <span className="text-[11px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-wider">Logic Flaws</span>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm">
        <div className="px-8 py-6 bg-slate-50 dark:bg-slate-700 border-b border-slate-200 dark:border-slate-600 flex flex-col xl:flex-row xl:items-center justify-between gap-6">
          <div className="shrink-0">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-300 font-medium mt-1">Live stream of AI interventions across all papers.</p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 flex-wrap">
            ...
            <div className="w-px h-10 bg-slate-200 dark:bg-slate-600 hidden sm:block mx-1" />
            <div className="space-y-2">
                  ...
                  <tr key={i} className="group hover:bg-slate-50/80 dark:hover:bg-slate-700/80 transition-all duration-300">
                    ...
                    <td className="px-8 py-5">
                      <span className={`px-2.5 py-1 rounded-lg text-[9px] font-black tracking-widest ${
                        log.severity === 'HIGH' ? 'bg-red-50 text-red-600 border border-red-100 dark:bg-red-900 dark:text-red-300 dark:border-red-800' : 
                        log.severity === 'MEDIUM' ? 'bg-amber-50 text-amber-600 border border-amber-100 dark:bg-amber-900 dark:text-amber-300 dark:border-amber-800' : 
